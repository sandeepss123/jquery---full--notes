...
;(function($){
	var ZoompleOverlay = (function(){
		...
		function init(){
			...
			function setEyvarSize(width, height){
				$eyvar.css({width : width, height : height});
				eyvarSize.width = width;
				eyvarSize.height = height;
			}
			function getEyvarSize(){
				return eyvarSize;
			}
			function moveEyvar(css){
				return $eyvar.css(css);
			}
			function clearEyvar(){
				
				return $eyvar.css({background : 'transparent'});
			}
			
			$overlay = $("<div id=\"zoomple_image_overlay\"><div class=\"eyevar\"></div></div>").appendTo("body");
			$eyvar = $overlay.find(".eyevar");
			...
			return {
				...
				getEl : getEl,
				setEyvarSize : setEyvarSize,
				getEyvarSize : getEyvarSize,
				getEyvar : getEyvar,
				moveEyvar : moveEyvar,
				clearEyvar : clearEyvar
			}
		}
		return{
	...
	Zoomple.prototype.positionZoom = function($target,e){
		...
		if(this.options.showOverlay){
			var eyvarPos = this.moveEyvar(Math.round(e.pageX - $target.offset().left) ,  Math.round(e.pageY - $target.offset().top)),
				width = $img.width(),
				height = $img.height();
			
			this.moveImageOverlay(-Math.round(width*eyvarPos.x),-Math.round(height*eyvarPos.y) , $img );
		}else{
	...
	Zoomple.prototype.delaiedZoom = function(imgRefUrl,imgDescription){
		...
		$(objImagePreloader).on("load", function() {	
			if(self.stopLoading){	
				...
				if(self.options.showOverlay) self.setupEyvar();
	...
	Zoomple.prototype.setupEyvar = function(){
		var eyvar = this.overlay.getEyvar(),
		...
		eyvar.css({background : "url("+img.attr('src')+") 0 0 no-repeat"});
		this.overlay.setEyvarSize(Math.round(ratioW*img.width()), Math.round(ratioH*img.height()));
	}
	Zoomple.prototype.clearEyvar = function(){
		this.overlay.clearEyvar();
	}
	Zoomple.prototype.moveEyvar = function(mouseX, mouseY){
		var eyvar = this.overlay.getEyvar(),
			size = this.overlay.getEyvarSize(),
		...
		this.overlay.moveEyvar({top : top, left : left});
		eyvar.css({backgroundPosition : -left+"px "+ -top+"px"});
